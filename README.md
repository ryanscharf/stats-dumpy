...
- **Aggregate across seasons** — appears once 0 or 2+ seasons are selected (a single season has nothing to aggregate). Off by default: each selected season stays its own row (`split_by_seasons = TRUE` sent to ASA), same as picking one season at a time. Turn it on to combine the selected seasons into one row per player-team instead. Since ASA's response doesn't include a `season_name` at all once seasons are combined, the app re-adds it as a concatenated label of the seasons involved (e.g. `2025-26, 2024-25`, or `All seasons` when Seasons was left blank).
- **Aggregate across teams** — independent of the seasons toggle, and always shown (unlike it, there's no reliable "more than one team" signal to gate visibility on ahead of a fetch). Off by default: a player who changed teams keeps one row per team (`split_by_teams = TRUE`). Turn it on to combine team stints into one row too (`split_by_teams = FALSE`) — with both toggles on, a player gets one true career-total row. With only this one on and seasons left split, a player gets one combined-team row *per season* instead. `team_id`/`team_abbreviation` are dropped the same way `season_name` is when combined, but with nothing to reconstruct which teams from, so a team-aggregated player's card just shows no team badge. Leaving this off while seasons are aggregated is a legitimate combination too — it's what put the same player back on the board as more than one card in the first place (one per team), before this toggle existed to make that a choice instead of a surprise.
