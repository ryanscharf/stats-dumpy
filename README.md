...
- **Aggregate across seasons** — appears once 0 or 2+ seasons are selected (a single season has nothing to aggregate). Off by default: each selected season stays its own row (`split_by_seasons = TRUE` sent to ASA), same as picking one season at a time. Turn it on to combine the selected seasons into one true career-total row per player instead — this also combines team splits (`split_by_teams = FALSE`), so a player who changed teams across the aggregated seasons still gets exactly one row, not one per team. (Combining seasons but leaving team splits on would otherwise put that player back on the board as what looks like a duplicate card, since nothing else distinguishes the two rows.) Since ASA's response doesn't include a `season_name` at all once seasons are combined, the app re-adds it as a concatenated label of the seasons involved (e.g. `2025-26, 2024-25`, or `All seasons` when Seasons was left blank) — `team_id`/`team_abbreviation` are dropped the same way, but with nothing to reconstruct them from, so an aggregated multi-team player's card just shows no team badge.
